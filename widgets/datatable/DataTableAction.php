...
class DataTableAction extends Action
{
    ...
    public function run()
    {
        ...
        try {
            $response = [
                ...
                'data' => $this->getData($filterQuery, $columns),
            ];
        } catch (\Exception $e) {
    ...
    public function getData(ActiveQuery $query, $columns){
        $data = $this->formatData($query, $columns);
        foreach($data as $k => $d){
            $cdata = [];
            foreach($columns as $column){
                ...
                if(strpos($column['data'],'.')){
                    $posdot = explode('.',$column['data']);
                    $cdata[$posdot[0]][$posdot[1]] = ArrayHelper::getValue($d, $column['data']);
                }else {
                    $cdata[$column['data']] = ArrayHelper::getValue($d, $column['data']);
                }
            }
            $data[$k] = $cdata;
            //print_r(ArrayHelp
            //er::getValue($d,'refJenis.kodename'));
        }
        return $data;
        //print_r($data);
    }
    ...
    public function applyFilter(ActiveQuery $query, $columns, $search)
    {
        ...
        foreach ($columns as $column) {
            if ($column['searchable'] == 'true' && array_key_exists($column['name'], $schema) !== false) {
                ...
                //$data_search = (($column['name'])?$column['name']:$column['name']);
                //echo ($this->controller->getUniqueId().'-dt-'.$column['name']);
                //Yii::$app->session->set($this->controller->getUniqueId().'-dt-'.$column['name'],$value);
                $query->andFilterWhere(['like', $column['name'], $value]);
            }
        }
        return $query;
    }
